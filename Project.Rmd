...
summary(subset(IL.contributor, party == 'republican'),maxsum = 20) 
```
The republican party have a lot of members and their distributions are much more
spread out. But again to draw better conclusions we need to look at the numbers.
```{r, echo=FALSE}
democratic <- subset(IL, party == 'republican' ) %>%
  group_by(party, cand_nm) %>%
  summarise(amount = sum(contb_receipt_amt),
            mean = mean(contb_receipt_amt),
            median = median(contb_receipt_amt),
...
Ok, so the numbers look close enough. But that is too much information for a 
general comparison. Lets plot a preqpoly of the two candidates on the amount
that they cumulatively received.
