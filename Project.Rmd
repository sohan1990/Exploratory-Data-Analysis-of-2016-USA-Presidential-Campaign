...
### Contribution scaled by dot size
```{r}
# scaling dot size
...
locations, as in zip codes to be exact. We can group these ans view them as 
...
But there is no relativity here. Only a scale on which the contributions have
varied from one county to the other.

To add the human factor here, we need to look at the ratio of contribution to
